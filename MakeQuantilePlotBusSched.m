...
%myOracleName = 'DiscrQuad';

strreps = '5'; %string with number of replications, for loading data files
...
for j=1:rowsubs  
    for i=1:colsubs
        ...
        titl_string = ['\fbox{\textbf{',dim,' Bus','}}'];%: ',myqstr,'}}'];
        titl_stringlgd = [myqstr ' %-iles'];
        %title(titl_string,'Interpreter','latex','fontsize',fs);
        if j==1 && i==1
            lgd=legend([p1 p2],{'R-SPLINE','ADALINE'},'Position',[0.3270 0.8260 0.1549 0.0919]);
            title(lgd,titl_stringlgd)
            title(titl_string,'Interpreter','latex','fontsize',fs2);
        elseif j==1 && i==2
            %lgd=legend([p1 p2],{'R-SPLINE','ADALINE'},'Location','best');%'Position',[0.8310 0.8413 0.1549 0.0919]) ;
            title(lgd,titl_stringlgd)
            title(titl_string,'Interpreter','latex','fontsize',fs2);
        elseif j==2 && i==1
