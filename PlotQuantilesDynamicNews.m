...
myOracleName = 'DynamicNews';

strreps = '100'; %string with number of replications, for loading data files
...
for j=1:2%rowsubs  
    for i=1:2%colsubs
        ...
        ylabel('\% Opt. Gap in $\mathcal X$','Interpreter','latex','fontsize',fs);
        myqstr = '';
        for k = 1:length(myq)
            nextq = num2str(myq(k));
            if k<length(myq)
        ...
        titl_string = ['\fbox{\textbf{Dynamic Newsvendor, $d=$ ',dim,'}}'];%: ',myqstr,'}}'];
